...
- **Media resolution guard**: `askQuestion` fetches MediaFiles scoped to `userId`; if `files.length !== mediaIds.length` → 400 `INVALID_MEDIA`. PDFs → `document` URL blocks, images → `image` URL blocks (files are served from local disk `/uploads`, ingested by their static URL, no base64). Media attached only to the **latest** user turn.
