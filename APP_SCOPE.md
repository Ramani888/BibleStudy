...
- Library: **`react-native-purchases` (RevenueCat)** — unifies StoreKit/Play; purchases go through the RC SDK.
- Backend: **RevenueCat S2S webhook is the sole entitlement source** (`POST /subscriptions/rc-webhook`) — it sets `User.plan`/`storageLimit` + grants credits via a `CreditTransaction`, gated by a durable per-transaction ledger. Legacy server-side receipt verification (`/verify`) was removed in the #20 hardening (2026-09-16). Entitlement is dual per-environment (prod precedence, sandbox = entitlement-only/no-credits) and fully expiry-aware.
...
| **E. Subscriptions** ✅ BUILT + HARDENED (2026-09-16, #20) | **RevenueCat** (`react-native-purchases`) + RC S2S webhook as the sole entitlement source, plan→benefit mapping, ledger-idempotent credit grants, dual per-env entitlement (prod precedence / sandbox no-credits), expiry-aware quota/retention, consumer Premium, paywall UX. Legacy `/verify` removed. Pending real-world config only (RC project + store products + `RC_WEBHOOK_AUTH`). (**Church/group plans deferred post-launch — depend on groups being rebuilt.**) | The money. Only converts once C+D make the app sticky. | L |
...
| **G. Enforcement & polish** ✅ G1–G3 DONE | Per-tier rate limits + storage bumps landed in E-a. G1 media full-cost-upfront (reject before Claude, no floor), G2 over-quota banner→Paywall on Profile, G3 out-of-credits Upgrade CTA in AI chat. **G4 S2S renewal webhooks ✅ DONE** — the RC webhook is now the primary entitlement path (idempotent grants, out-of-order/expiry handling), shipped in the #20 hardening. | Hardens the paid tiers after they exist. | M |
| **H. Growth loop** ✅ SHIPPED 2026-09-15 (backend live, prod-migrated) | From market research ([[growth_revenue_research]]): **H1 PostHog analytics** (US cloud, explicit events + opt-out, retention visibility). **H2 referral rewards** (invite code + redeem, both sides +5, cap 25 — see [[Module - AI & Credits]]). **H3 more WhatsApp share surfaces** (quiz result, streak, achievement, plan — each carries the referral link). **H4 Portuguese device-default** (Brazil-first). **H5 AI-differentiator copy** (onboarding + paywall). | Distribution-first bet — WhatsApp virality in Brazil is the only near-zero-budget path per research. Referral device-tested (both +5). **Remaining: app release build for real users; pt native review.** | M |
